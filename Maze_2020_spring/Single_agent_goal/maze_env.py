...
class GridWorldEnv(Env):
    ...
    def __init__(self,
        ...
        self.obs_states = None  # empty or obstacles
    ...
    def step(self, action):
        ...
        # get the reward
        if agent_state_cell in self.visited and collision_mark:
            reward = -10  # pass the visited ceil
        if done:
            reward = 50     # goal
    ...
    def get_local_state(self, states):
        """
        ...
        distance = np.abs(self.agent_state[0] - self.goal_pos[0]) \
                   + np.abs(self.agent_state[1] - self.goal_pos[1])
        states.extend(local_state)
        start_goal = [self.start[0], self.start[1], self.goal_pos[0], self.goal_pos[1]]
        states.extend(start_goal)
        states.append(distance)

    def reset(self, states, show=False):
        """
        get the local states
        """
        self.agent_state = self.start  # the start
        self.visited.clear()
        # self.goal_pos = self.ends[0]
        if show:
            self.goal_pos = show
        else:
            self.goal_pos = sample(self.ends, 1)[0]
        # print(self.goal_pos)
        # input('123')
        self.get_local_state(states)
    ...
    # arrive the goal
    def _is_end_state(self, x, y=None, show=False):
        if y is not None:
            xx, yy = x, y
        ...
        if show:
            ...
            if xx == self.goal_pos[0] and yy == self.goal_pos[1]:
                return True
        return False
        ...
        if self.viewer is None:
            ...
            for x in range(self.n_width):
                for y in range(self.n_height):
                    ...
                    if self._is_end_state(x, y, show=True):
...
def maze_grid(start, ends, n_height, n_width,obstacles):
    '''
