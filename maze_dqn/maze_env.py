...
class GridWorldEnv(Env):
    ...
    def step(self, action):
        ...
        # 类型为0的格子为障碍格子，不可进入
        if self.grids.get_type(new_x, new_y) == 0:
            new_x, new_y = old_x, old_y
        done = self._is_end_state(new_x, new_y)
        agent_state_cell = self._xy_to_state(self.agent_state)
        # get the reward
        if agent_state_cell in self.visited:
            reward = -1  # pass the visited ceil
        elif done:
            reward = 10     # goal
        else:
            reward = -0.1  # pass a ceil

        if agent_state_cell not in self.visited:
            self.visited.add(agent_state_cell)  # the past cell

        self.agent_state = (new_x, new_y)
        goal_pos = self.ends[0]
        distance = np.abs(new_x-goal_pos[0]) + np.abs(new_y-goal_pos[1])
        env_states = [new_x, new_y, goal_pos[0], goal_pos[1], distance]
        # calculate the distance between agent and obstacle
        self.cal_distance_agent_obstacle(env_states)
        # 提供格子世界所有的信息在info内
        info = {"x": new_x, "y": new_y, "grids": self.grids}
        return env_states, reward, done, info

    def cal_distance_agent_obstacle(self, env_states):
        for obs in self.obstacles:
            env_states.extend([obs[0], obs[1]])
            dis = np.abs(self.agent_state[0]-obs[0]) + np.abs(self.agent_state[1]-obs[1])
            env_states.append(dis)
    ...
    def refresh_setting(self):
        ...
        for x, y, t in self.types:
            self.grids.set_type(x, y, t)  # color
        env_states = self.reset()
        self.observation_space = spaces.Discrete(len(env_states))

    def reset(self):
        # obs_states = self.get_map_state()
        self.agent_state = self.start  # the start
        self.visited = set()
        agent_pos = self.agent_state
        goal_pos = self.ends[0]
        distance = np.abs(agent_pos[0]-goal_pos[0]) + np.abs(agent_pos[1]-goal_pos[1])
        env_states = [agent_pos[0], agent_pos[1], goal_pos[0], goal_pos[1], distance]
        self.cal_distance_agent_obstacle(env_states)
        return env_states
            ...
            for x in range(self.n_width):
                for y in range(self.n_height):
                    ...
                    if self.start[0] == x and self.start[1] == y:
                        ...
                        self.viewer.add_geom(outline)
                    if self.grids.get_type(x, y) == 0:  # 障碍格子用深灰色表示, obstacle cells are with gray color
...
def maze_grid(start, ends, obstacles):
    '''10*10的一个格子世界环境，设置参照：
    ...
    for i in ends:
        i_list = list(i)
        i_list.append(1)
        env.rewards.append(tuple(i_list))   # the goal has special color
...
if __name__ == "__main__":
    start = (2, 9)
    ends = [(5, 4)]  # many ends
    obstacles = [(4, 2), (4, 3), (4, 4), (4, 5), (4, 6), (1, 7), (2, 7), (3, 7), (4, 7), (6, 7), (7, 7), (8, 7)]
    env = maze_grid(start, ends, obstacles)  # 10*10 maze
    env.render()
    input('123')
